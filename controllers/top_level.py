...
def area_validation(form):
    ...
    if p_records:
        ...
        row=db(
            (db.sm_top_level.cid==session.cid) & 
            (db.sm_top_level.level_id==level0)
        ).select(
            db.sm_top_level.ALL,
            orderby=~db.sm_top_level.id,
        ).first()
...
def zone_validation(form):
    ...
    if p_records:
        ...
        row=db(
            (db.sm_top_level.cid==session.cid) & 
            (db.sm_top_level.level_id==level1)
        ).select(
            db.sm_top_level.ALL,
            orderby=~db.sm_top_level.id,
        ).first()
...
def territory_validation(form):
    ...
    if len(existRecord) > 0:
        ...
        if p_records:
            ...
            row=db(
                (db.sm_top_level.cid==session.cid) & 
                (db.sm_top_level.level_id==level2)
            ).select(
                db.sm_top_level.ALL,
                orderby=~db.sm_top_level.id,
            ).first()
...
def territory():
    ...
    inusedRecords = db(
        ...
        groupby=db.sm_top_level.level3
    )
    ...
    db.sm_top_level.level_name.requires=IS_IN_SET(territory_list)
...
def download_excel():
    ...
    if btn_download and search_type == 'ZoneID':
        searchValue = str(search_value).split('|')[0].strip()
        qset &= (db.sm_top_level.level2 == searchValue.upper())
        
    if btn_download and search_type == 'TerritoryID':
        searchValue = str(search_value).split('|')[0].strip()
        qset &= (db.sm_top_level.level3 == searchValue.upper())
