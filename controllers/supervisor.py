def supervisor_validation(form):
    pass
...
def index():
    ...
    if btn_download:
        ...
        session.search_value=search_value
        reqPage=0
        redirect(URL(c='supervisor', f='download_excel',vars=dict(
    ...
    allowed_designation = [u.field1 for u in db((db.sm_category_type.cid == session.cid) & ~(db.sm_category_type.cat_type_id.belongs(['D005','D007'])) & (db.sm_category_type.type_name == 'EMP_DESIGNATION')).select(db.sm_category_type.cat_type_id,db.sm_category_type.field1,orderby=db.sm_category_type.cat_type_id)]
        
    db.sm_rep.note.requires=IS_IN_SET(allowed_designation)
    db.sm_rep.status.requires=IS_IN_SET(('ACTIVE','INACTIVE'))
    db.sm_rep.status.default='ACTIVE'
    form =SQLFORM(db.sm_rep,
        fields=['note','rep_id','name','rep_username','mobile_no','status'],
        submit_button='Save'
    )
    
    if form.accepts(request.vars,session,onvalidation=supervisor_validation):
